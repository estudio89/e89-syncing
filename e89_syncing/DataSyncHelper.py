...
def saveNewData(user, timestamp, timestamps, device_id, data, files, platform = None):
	...
	new_data = getModifiedData(user = user, timestamp = timestamp, timestamps = timestamps, exclude = new_objects, platform = platform)
	response.update(new_data)

	return response

def getModifiedData(user, timestamps, timestamp = None, exclude = {}, platform = None):
	...
	for sync_manager in E89SyncingConfig.get_sync_managers():
		...
		manager_data,manager_parameters = sync_manager.getModifiedData(user = user, timestamp = timestamp, exclude = exclude.get(identifier,[]), platform = platform)
...
def getModifiedDataForIdentifier(user, parameters, identifier, timestamps, platform = None):
	...
	manager_data,manager_parameters = sync_manager.getModifiedData(user = user, timestamp = timestamp_to_datetime(timestamp), parameters = parameters, platform = platform)
	manager_parameters['data'] = manager_data
	...
	if timestamps.has_key(identifier): # Not paginating, but updating
		...
		if not coupled:
			...
			for sm in coupled:
				...
				coupled_data,coupled_parameters = coupled_sync_manager.getModifiedData(user = user, timestamp = timestamp_to_datetime(timestamp), parameters = parameters, platform = platform)
...
class BaseSyncManager(object):
	...
	def getModifiedData(self, user, timestamp, parameters = None, exclude = [], platform = None):
	...
	def saveNewData(self, user, device_id, data, files = None, platform = None):
		...
		new_objects = []
		for object in data:
			object_response, new_object = self.saveObject(user = user, device_id = device_id, object = object, files = files, platform = platform)
			response.append(object_response)
			new_objects.append(new_object)

		return {self.getResponseIdentifier():response},new_objects

	def saveObject(self, user, device_id, object, files = None, platform = None):
